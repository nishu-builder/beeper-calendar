...
pub fn save_secret_value(service: &str, token: &str) -> Result<(), String> {
    if !["beeper", "github"].contains(&service) {
        return Err("Unknown credential.".into());
    }
    let entry =
        keyring::Entry::new(SERVICE, service).map_err(|_| "Credential store unavailable.")?;
    if token.is_empty() {
        entry
            .delete_credential()
            .or_else(|e| {
            ...
            .map_err(|_| "Could not remove credential.".to_string())
    } else {
        entry
            .set_password(token)
...
fn secret(service: &str) -> Result<String, String> {
    keyring::Entry::new(SERVICE, service)
        .and_then(|e| e.get_password())
        .map_err(|_| {
            format!(
                "Connect {} in Settings. Its credential is missing or the keychain is locked.",
                service
            )
        })
}
#[tauri::command]
fn save_secret(service: String, token: String) -> Result<(), String> {
    save_secret_value(&service, &token)
}
#[tauri::command]
fn credential_status() -> serde_json::Value {
    serde_json::json!({"beeper":secret("beeper").is_ok(),"github":secret("github").is_ok()})
}
/// Import an existing GitHub CLI sign-in without exposing its token to JavaScript.
#[tauri::command]
async fn connect_github_cli() -> Result<(), String> {
    ...
    for executable in candidates {
        ...
        match result {
            Ok(Ok(output)) if output.status.success() => {
                let token = String::from_utf8(output.stdout).map_err(|_| "GitHub CLI returned an invalid credential.".to_owned())?;
                let token = token.trim();
                if token.is_empty() || token.len() > 4096 || token.contains(char::is_whitespace) {
                    return Err("GitHub CLI returned an invalid credential.".into());
                }
                return save_secret_value("github", token);
            }
            Ok(Err(error)) if error.kind() == std::io::ErrorKind::NotFound => continue,
            Err(_) => return Err("GitHub CLI sign-in timed out. Use a token instead or check your GitHub CLI sign-in.".into()),
            _ => return Err("Sign in to GitHub CLI first, or use a repository-scoped token below.".into()),
        }
    }
    Err("GitHub CLI is not installed. Use a repository-scoped token below.".into())
}
#[derive(Deserialize)]
...
fn allowed_url(request: &Request) -> Result<url::Url, String> {
    ...
    {
        return Err("Service URLs cannot contain credentials, queries or fragments.".into());
    }
    ...
    if parsed.origin() != base.origin() {
        return Err("Cross-origin requests are blocked.".into());
    }
    let path = parsed.path();
    match request.service.as_str() {
        "github" => {
            if base.as_str() != "https://api.github.com/" || !path.starts_with("/repos/") {
                return Err("Only the GitHub repository API is supported.".into());
            }
            if !["GET", "POST", "PATCH", "PUT"].contains(&request.method.as_str()) {
        ...
        "beeper" | "ollama" => {
            ...
            {
                return Err(
                    "Beeper and the model must run locally over a loopback HTTP address.".into(),
                );
            }
            let allowed = if request.service == "beeper" {
                (request.method == "GET"
                    && (path == "/v1/info"
                        ...
                        || path.starts_with("/v1/chats/")))
                    || (request.method == "POST" && path == "/v1/focus")
            } else {
                (request.method == "GET" && path == "/api/tags")
                    || (request.method == "POST" && path == "/api/chat")
            };
            if !allowed {
                return Err("This service operation is blocked.".into());
            }
        }
...
async fn service_request(request: Request) -> Result<Response, String> {
    ...
    let client = reqwest::Client::builder()
        ...
        .no_proxy()
        .timeout(Duration::from_secs(if request.service == "ollama" {
            180
        } else {
            30
        }))
        .build()
        .map_err(|_| "Could not start HTTP client.")?;
    let method = reqwest::Method::from_bytes(request.method.as_bytes())
        .map_err(|_| "Invalid HTTP method.")?;
    let mut builder = client
        .request(method, url)
        .header("User-Agent", "Beeper-Calendar/0.1");
    if request.service != "ollama" {
        builder = builder.bearer_auth(secret(&request.service)?);
    }
    ...
    let mut response = builder.send().await.map_err(|e| {
        if e.is_timeout() {
            "Service timed out. Check that it is running, then retry.".to_string()
        } else {
            "Could not reach the service. Check its address and connection in Settings.".to_string()
        }
    })?;
    ...
    while let Some(chunk) = response
        .chunk()
        .await
        .map_err(|_| "Interrupted service response.")?
    {
        if bytes.len() + chunk.len() > 12_000_000 {
            return Err(
                "Service response exceeds the size limit. Choose a smaller calendar repository."
                    .into(),
            );
        }
...
fn state_path(app: &tauri::AppHandle) -> Result<std::path::PathBuf, String> {
    let dir = app
        .path()
        .app_data_dir()
        .map_err(|_| "Application storage unavailable.")?;
    fs::create_dir_all(&dir).map_err(|_| "Could not create application storage.")?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        fs::set_permissions(&dir, fs::Permissions::from_mode(0o700))
            .map_err(|_| "Could not protect application storage.")?;
    }
...
fn load_state(
    ...
    match fs::read(path) {
        Ok(bytes) => serde_json::from_slice(&bytes).map(Some).map_err(|_| {
            "Saved state is unreadable. Preserve the state file before recovery.".into()
        }),
...
fn save_state(
    ...
    if bytes.len() > 20_000_000 {
        return Err("Local history is full. Remove completed items before continuing.".into());
    }
    ...
    let mut file = options
        .open(&temp)
        .map_err(|_| "Could not save local state.")?;
    file.write_all(&bytes)
        .and_then(|_| file.sync_all())
        .map_err(|_| "Could not persist local state.")?;
    fs::rename(&temp, &path).map_err(|_| "Could not commit local state.")?;
    #[cfg(unix)]
    fs::File::open(path.parent().unwrap())
        .and_then(|f| f.sync_all())
        .map_err(|_| "Could not sync application storage.")?;
    Ok(())
}
#[tauri::command]
fn read_clipboard(app: tauri::AppHandle) -> Result<String, String> {
    let text = app
        .clipboard()
        .read_text()
        .map_err(|_| "Copy a text message in Beeper first.")?;
    if text.len() > 16_000 {
        return Err("Copy a shorter message (under 16,000 characters).".into());
    }
    Ok(text)
}
#[tauri::command]
fn open_link(app: tauri::AppHandle, url: String) -> Result<(), String> {
    ...
    if parsed.scheme() != "https"
        || !matches!(
            parsed.host_str(),
            Some("github.com" | "calendar.google.com")
        )
    {
...
fn show(app: &tauri::AppHandle, capture: bool) {
    ...
    if capture {
        let _ = app.emit("capture-message", ());
    }
}
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let capture =
                MenuItem::with_id(app, "capture", "Use copied message", true, None::<&str>)?;
            let quit = MenuItem::with_id(app, "quit", "Quit", true, None::<&str>)?;
            let menu = Menu::with_items(app, &[&open, &capture, &quit])?;
            let mut tray = tauri::tray::TrayIconBuilder::new()
                .menu(&menu)
                .tooltip("Beeper Calendar")
                .on_menu_event(|app, event| match event.id().as_ref() {
                    "open" => show(app, false),
                    "capture" => show(app, true),
                    "quit" => app.exit(0),
                    _ => {}
                });
...
mod tests {
    use super::*;
    fn req(service: &str, base: &str, path: &str, method: &str) -> Request {
        Request {
